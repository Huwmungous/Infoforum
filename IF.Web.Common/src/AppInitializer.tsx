...
import { ConfigService, AppType } from './configServiceClient';
...
import { getCurrentRoutePath, buildAppUrl, getAppBasePath } from "./routing";
...
/**
 * AppInitializer is the unified entry point for SfD applications.
      ...
      try {
        ...
        if (staticConfig) {
          ...
          ConfigService.initializeWithStaticConfig({
            ...staticConfig,
            realm: EnvironmentConfig.get('SFD_REALM'),
            appName: EnvironmentConfig.get('SFD_APP_NAME'),
            environment: EnvironmentConfig.get('SFD_ENVIRONMENT'),
            appType
          });
        } else {
          ...
          await ConfigService.initialize({
            configServiceUrl: EnvironmentConfig.get('SFD_CONFIG_SERVICE_URL'),
            realm: EnvironmentConfig.get('SFD_REALM'),
            client: EnvironmentConfig.get('SFD_CLIENT'),
            appType,
            appName: EnvironmentConfig.get('SFD_APP_NAME'),
            environment: EnvironmentConfig.get('SFD_ENVIRONMENT')
          });
