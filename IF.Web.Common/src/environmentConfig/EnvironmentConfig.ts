...
export class EnvironmentConfig {
  ...
  public static get(key: string): string {
    ...
    try {
      ...
      // Special case: SFD_ENVIRONMENT can be derived from Vite's MODE (--mode flag)
      // Only return MODE if it's a valid environment value
      // @ts-ignore
      if (key === 'SFD_ENVIRONMENT' && typeof import.meta !== 'undefined' && import.meta.env && import.meta.env.MODE) {
        // @ts-ignore
  ...
  public static validate(useStaticConfig: boolean): string[] {
    ...
    const requiredForAll = ['SFD_REALM', 'SFD_CLIENT', 'SFD_APP_NAME', 'SFD_ENVIRONMENT', 'LOG_LEVEL'];
    const requiredForDynamic = ['SFD_CONFIG_SERVICE_URL'];
    ...
    // Validate SFD_ENVIRONMENT is one of the allowed values
    const environment = this.get('SFD_ENVIRONMENT');
    if (environment && !this.VALID_ENVIRONMENTS.includes(environment.toUpperCase())) {
      errors.push(`Invalid SFD_ENVIRONMENT: '${environment}'. Must be one of: ${this.VALID_ENVIRONMENTS.join(', ')}`);
    }
  ...
  public static logConfiguration(useStaticConfig: boolean): void {
    const commonKeys = ['SFD_REALM', 'SFD_CLIENT', 'SFD_APP_NAME', 'SFD_ENVIRONMENT', 'LOG_LEVEL'];
    const dynamicKeys = ['SFD_CONFIG_SERVICE_URL'];
