  ...
  EnvironmentConfig: {
    get: vi.fn((key: string) => {
      const envVars: Record<string, string> = {
        SFD_APP_NAME: 'TestApp',
        SFD_ENVIRONMENT: 'DEV',
        SFD_REALM: 'test-realm',
        SFD_CLIENT: 'test-client',
        SFD_CONFIG_SERVICE_URL: 'https://config.example.com',
      };
      return envVars[key] || '';
    }),
    validate: vi.fn(() => []),
...
describe('AppInitializer', () => {
  ...
  describe('validation errors', () => {
    it('should show error when validation fails', async () => {
      (EnvironmentConfig.validate as any).mockReturnValue(['Missing: SFD_APP_NAME']);
      ...
      expect(EnvironmentConfig.logValidationErrors).toHaveBeenCalledWith(['Missing: SFD_APP_NAME']);
  ...
  describe('appType prop', () => {
    ...
    it('should accept "patient" appType', async () => {
      render(
        <AppInitializer appType="patient">
          <div>Content</div>
        </AppInitializer>
      );
      ...
      await waitFor(() => {
        expect(ConfigService.initialize).toHaveBeenCalledWith(
          expect.objectContaining({ appType: 'patient' })
        );
      });
    });
  });
