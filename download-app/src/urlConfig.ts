 ...
 * 
 * Examples:
 *   https://longmanrd.net/infoforum/downloads/
 *   http://localhost:5315/infoforum/downloads/
 * 
...
export function getConfigFromUrl(): UrlConfig | null {
  ...
  // Pattern: /{appDomain}/downloads/...
  const match = pathname.match(/^\/([^/]+)\/downloads/i);
 ...
 * @returns Base path like "/infoforum/downloads"
 */
export function getAppBasePath(urlConfig: UrlConfig): string {
  return `/${urlConfig.appDomain}/downloads`;
}
 ...
 * @returns Full URL like "https://longmanrd.net/infoforum/downloads/signin/callback"
 */
