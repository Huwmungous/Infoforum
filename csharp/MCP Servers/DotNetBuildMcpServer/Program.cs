using DotNetBuildMcpServer.Protocol;
...
builder.Services.AddSingleton<SfdLogger>(sp =>
{
    ...
    return new SfdLogger("DotNetBuildMcpServer", config.LoggerServiceUrl, config.ClientId, config.Realm);
});
...
app.UseSwaggerUI(c =>
{
    c.SwaggerEndpoint("/swagger/v1/swagger.json", "DotNetBuildMcpServer v1");
...
app.UseSwaggerUI(c =>
{
    c.SwaggerEndpoint("/swagger/v1/swagger.json", "DotNetBuildMcpServer API v1");
    c.RoutePrefix = "swagger";
});


app.Logger.LogInformation("DotNet Build MCP Server listening on port {Port}", port);
app.Run();