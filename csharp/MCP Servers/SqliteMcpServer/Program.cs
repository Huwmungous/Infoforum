...
builder.Services.AddSingleton<SfdLogger>(sp =>
{
    ...
    return new SfdLogger("SqliteMcpServer", config.LoggerServiceUrl, config.ClientId, config.Realm);
});
...
app.Logger.LogInformation("SQLite MCP Server listening on port {Port}", port);
app.Run();
