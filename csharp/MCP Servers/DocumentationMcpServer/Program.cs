...
// File: Program.cs
// ============================================
using DocumentationMcpServer.Protocol; 
...
builder.Services.AddSingleton<SfdLogger>(sp =>
{
    ...
    return new SfdLogger("DocumentationMcpServer", config.LoggerServiceUrl, config.ClientId, config.Realm);
});
...
app.UseSwaggerUI(c =>
{
    c.SwaggerEndpoint("/swagger/v1/swagger.json", "DocumentationMcpServer API v1");
    c.RoutePrefix = "swagger";
});


app.Logger.LogInformation("Documentation MCP Server listening on port {Port}", port);
app.Run();
