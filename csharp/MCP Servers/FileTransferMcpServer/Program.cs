using FileTransferMcpServer;
using FileTransferMcpServer.Protocol;
...
builder.Services.AddSingleton<SfdLogger>(sp =>
{
    ...
    return new SfdLogger("FileTransferMcpServer", config.LoggerServiceUrl, config.ClientId, config.Realm);
});
...
app.UseSwaggerUI(c =>
{
    c.SwaggerEndpoint("/swagger/v1/swagger.json", "FileTransferMcpServer API v1");
    c.RoutePrefix = "swagger";
});


app.Logger.LogInformation("FileTransfer MCP Server listening on port {Port}", port);
app.Run();
