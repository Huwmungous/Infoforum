...
builder.Services.AddSingleton<SfdLogger>(sp =>
{
    ...
    return new SfdLogger("SqlGeneratorMcpServer", config.LoggerServiceUrl, config.ClientId, config.Realm);
});
...
app.UseSwaggerUI(c =>
{
    c.SwaggerEndpoint("/swagger/v1/swagger.json", "SqlGeneratorMcpServer v1");
...
app.Logger.LogInformation("SQL Generator MCP Server listening on port {Port}", port);
app.Run();
