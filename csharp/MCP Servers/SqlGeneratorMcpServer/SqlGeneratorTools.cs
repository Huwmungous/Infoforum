...
public static class SqlGeneratorTools
{
    public static Task<object> GenerateSelect(JsonElement args)
    {
        ...
        var columns = args.TryGetProperty("columns", out var c) ? [.. c.EnumerateArray().Select(x => x.GetString()!)] : new[] { "*" };
    ...
    public static Task<object> TranslateSql(JsonElement args)
    {
        ...
        if (sourceDialect.Equals("firebird", StringComparison.CurrentCultureIgnoreCase) && targetDialect.Equals("sqlserver", StringComparison.CurrentCultureIgnoreCase))
        {
    ...
    public static Task<object> GenerateStoredProcCall(JsonElement args)
    {
        ...
        var parameters = args.TryGetProperty("parameters", out var p) ? [.. p.EnumerateArray().Select(x => x.GetString()!)] : Array.Empty<string>();
    ...
    public static Task<object> GenerateRepositoryInterface(JsonElement args)
    {
        ...
        var operations = args.TryGetProperty("operations", out var ops)
            ? [.. ops.EnumerateArray().Select(x => x.GetString()!)]
