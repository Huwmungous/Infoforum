...
builder.Services.AddSingleton<SfdLogger>(sp =>
{
    ...
    return new SfdLogger("GitMcpServer", config.LoggerServiceUrl, config.ClientId, config.Realm);
});
...
app.UseSwaggerUI(c =>
{
    c.SwaggerEndpoint("/swagger/v1/swagger.json", "GitMcpServer v1");
    c.RoutePrefix = "swagger";
});

app.Logger.LogInformation("Git MCP Server listening on port {Port}", port);
app.Run();