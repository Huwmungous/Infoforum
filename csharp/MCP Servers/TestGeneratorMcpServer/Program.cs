...
builder.Services.AddSingleton<SfdLogger>(sp =>
{
    ...
    return new SfdLogger("TestGeneratorMcpServer", config.LoggerServiceUrl, config.ClientId, config.Realm);
});
...
var sseHub = new SfD.Mcp.Protocol.Models.SseHub();

app.MapGet("/health", () => Results.Ok(new
{
    status = "healthy",
    service = "testgenerator-mcp",
    port,
    version = "1.0.0"
}));
...
app.Logger.LogInformation("Test Generator MCP Server listening on port {Port}", port);
app.Run();
