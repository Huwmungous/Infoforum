...
builder.Services.AddSingleton<SfdLogger>(sp =>
{
    ...
    return new SfdLogger("BraveSearchMcpServer", config.LoggerServiceUrl, config.ClientId, config.Realm);
});
...
app.UseSwaggerUI(c =>
{
    c.SwaggerEndpoint("/swagger/v1/swagger.json", "BraveSearchMcpServer v1");
...
app.Logger.LogInformation("Brave Search MCP Server listening on port {Port}", port);
app.Logger.LogInformation("Endpoints: GET /health, POST /mcp");
app.Logger.LogInformation("Configure Brave API key in appsettings.json: BraveSearch:ApiKey");
