...
builder.Services.AddSingleton<SfdLogger>(sp =>
{
    ...
    return new SfdLogger("ConfigManagementMcpServer", config.LoggerServiceUrl, config.ClientId, config.Realm);
});
...
app.UseSwaggerUI(c =>
{
    c.SwaggerEndpoint("/swagger/v1/swagger.json", "ConfigManagementMcpServer v1");
...
app.Logger.LogInformation("Configuration Management MCP Server listening on port {Port}", port);
